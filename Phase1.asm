...
looptomoveagin:
  ...
  cmp al,77h ;check if w
  jz w

  cmp al,64h ;check if d
  jz d

  cmp al,61h ;check if a
  jz a

  cmp al,73h ;check if s
  jz s

  cmp al,71h ;check if q
  jz q
  ...
  cmp al,6Dh ;check if m (select for the white player)
  jz m
...
; exit:               ;To close the game using esc after Gameover
;   mov ah,0
;   int 16h 
;   cmp al,27
;   jne exit
