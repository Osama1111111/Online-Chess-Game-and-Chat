...
hbly dw   0                
 ...
 FulltimeFilename DB 'images\clk3.bin', 0
...
looptomoveagin:
      ...
      mov RESAH, '$'
			mov AH, 1
			int 16H  
			jnz SendWhite ;Ther is a key pressed = sending ah or closing game
			JMP resBlack  ;No key pressed then recieving ah
...
SendWhite:
  ...
  cmp ah,48h ;check if up
  jnz pss52
  jmp upArrow
  pss52:

  cmp ah,4Dh ;check if right
  jnz pss28
  jmp rightArrow
  pss28:

  cmp ah,4Bh ;check if left
  jnz pss27
  jmp leftArrow
  pss27:
  ...
  cmp ah,2Bh ;check if m (select for the white player) (backslash)
  jnz pss25
  jmp  m
  pss25:
...
resBlack:
  ...
  cmp RESAH,48h ;check if w
  jz w

  cmp RESAH,4Dh ;check if d
  jnz pss12
  jmp d
  pss12:

  cmp RESAH,4Bh ;check if a
  jnz pss11
  jmp a
  pss11:

  cmp RESAH,50h ;check if s
  jnz pss10
  jmp s
  pss10:

  cmp RESAH,2Bh ;check if q (backSlash)
  jnz pss14
  jmp q
  pss14:
